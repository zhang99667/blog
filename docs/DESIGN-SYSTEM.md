...
- 博客正文右侧目录只在 `breakpoints.readingRail` 提供足够的双侧阅读净空后启用；该布局中互动栏位于正文左侧近邻，目录独占右侧。普通 `wide` 断点仍归笔记三栏布局，不能混用。
...
- 固定悬浮的文章互动栏必须跟随正文边缘，不能默认停在宽屏视口右下角。博客右侧目录吸顶时，互动栏改到正文左侧约一个标准间距并在视口中部固定；无独立目录栏时仍优先贴正文右侧，紧凑视口才退回安全区。笔记的可滚动目录末尾继续保留至少“互动栏高度 + 16px”的安全区，使最后一项完整可达。
