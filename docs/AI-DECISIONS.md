...
- 日期：2026-07-13；2026-07-14 交互纠偏；2026-07-16 身份与迁移纠偏
...
- 决策：博客成稿和公开笔记正文统一使用 Lucide `ArrowUp` 图标按钮；正文不足约一屏半或读者仍在首屏时隐藏，离开正文顶部约 0.65 个视口后才显示。按钮与互动栏共用正文右边缘或窄屏安全区锚点，但作为 44px 独立方形控件位于互动栏上方，不混入“文章数据”语义。激活时使用原生滚动，并把键盘焦点移到主阅读区；减少动态效果偏好下取消平滑滚动。按钮出现时，笔记目录尾部安全区同步扩展到整组工具高度再加 16px。
