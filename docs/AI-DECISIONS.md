...
- 锁定证据：reactions API 的点赞与浏览幂等、并发、旧库迁移和持久化测试，Lucide 精确依赖，Compose 无公网端口契约，Nginx 精确路由与写限流测试，部署脚本的 `--force-recreate --wait reactions` 顺序检查，两站正文首屏图标栏、多视口双主题和 SPA 浏览器用例，以及生产健康、幂等写入与端口 smoke。Lucide 图标数据使同时包含博客与笔记回退产物的 blog 总 JS 上限从 485 KB 调整到 490 KB；200 KB 首屏上限与 notes 总量上限保持不变。
