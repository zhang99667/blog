...
- 决策：博客内容仍是公开笔记中的精选成稿，但阅读面只复用成熟的 Quartz `TableOfContents`。`BlogFrame` 从既有右侧组件中只渲染目录，拒绝 Graph 与 Backlinks；达到 1400px 阅读栏断点时，目录作为正文右侧吸顶栏，互动栏改锚到目录外侧；空间不足时目录放在标题元数据之后、正文之前。长目录在有限高度内滚动并保留折叠交互，首页、归档和没有可生成目录的文章不显示占位区域。
