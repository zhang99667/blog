...
- 边界：同步重命名表示旧路径删除、新路径生成，不自动承诺旧 URL 重定向；目录映射得到同一 slug 时可保持路由，例如 `硕士` 改为 `master` 后仍为 `/master/`。博客精选若仍依赖 `blog.config.mjs` 的源路径，源文件改名需要更新配置，或把源笔记改为带稳定 slug 的 `type: post`。
