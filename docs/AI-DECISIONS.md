...
- 日期：2026-07-13
- 触发：用户要求在笔记或博客增加点赞功能。
- 决策：博客和笔记正文共用克制的单向点赞组件，按 `site + canonical slug` 独立计数。独立 `markz-reactions` 服务使用 SQLite 和唯一键保证持久化与幂等；浏览器随机 ID 经 SHA-256 后保存，来源 IP 只在 Nginx 限流内存中短暂使用，不进入数据库。服务没有宿主机端口，也不复用 JSONUtils API 或数据库。
- 反例：只在 `localStorage` 伪造公共计数、把点赞接口塞进 JSONUtils、为匿名点赞引入账号系统，或让数据库目录进入静态站 `rsync --delete` 范围。
- 边界：博客成稿与原始笔记是不同内容表面，默认不合并计数；清空浏览器存储后可再次点赞，当前不承诺账号级防刷。首页、目录、标签、Canvas 和 Bases 页面不展示点赞。
- 锁定证据：reactions API 幂等、并发和持久化测试，Compose 无公网端口契约，Nginx 精确路由与写限流测试，两站文章多视口双主题和 SPA 浏览器用例，以及生产健康、写入与端口 smoke。
