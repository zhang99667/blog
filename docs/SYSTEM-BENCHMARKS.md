...
| [CSSTools Cascade Layers](https://github.com/csstools/postcss-plugins/tree/main/plugins/postcss-cascade-layers) | 构建期处理完整 CSS bundle，并用普通选择器保留 layer 优先级 | 旧 Android WebView 不识别 `@layer` 时仍获得完整基础、组件和品牌样式 |
