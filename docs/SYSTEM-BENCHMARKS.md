...
| [GitHub workflow artifacts](https://docs.github.com/en/actions/concepts/workflows-and-actions/workflow-artifacts) | 工作流产物具有生命周期、下载路径和保留策略 | 只上传密文与 checksum，每日恢复点保留 90 天                  |

不采用：复制在线 WAL 主文件、把解密密钥与密文放在同一平台、复用部署 SSH 私钥、只检查备份文件存在、自动覆盖生产库。默认关闭异地工作流，审批后才创建专用 identity 和启用 Artifact；上传前必须对同一密文完成真实解密与 SQLite 恢复。
