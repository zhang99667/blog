...
- 用户明确不采纳的能力仍由原探针判定、保留原评分并显示为未达成，但不进入自动优先队列；只有新的明确决定可以恢复它。
...
10. 广义“继续优化”任务先按演进报告选择一个有边界的活跃能力，完成后重新运行报告，证明能力状态发生变化；明确不采纳项不重复请求批准。
...
D-022 已把异地 Artifact 备份记为用户明确不采纳：源码工具只作为手动休眠能力保留，不创建 identity、不设置定时任务、不启用 Artifact，也不再主动请求批准。只有用户以后明确反转 D-022，才恢复 D-021 的审批、加密和真实下载恢复流程。源码中出现工作流不等于能力完成；原探针与评分必须保持不变，报告显示“明确不采纳”而不是“已达成”。
...
7. 运行 `npm run evolve:report`，确认已完成能力从缺口队列移除，下一项由同一评分模型选出；用户明确不采纳的能力保留失败证据并单独展示，不计入完成或自动队列。
