...
if (!existsSync(nginxConfig) || !existsSync(edgeCompose)) {
...
run(ssh[0], [
  ...ssh.slice(1),
  host,
  `mkdir -p ${remoteDir} ${remoteNotesDir} ${remoteAppDir} ${remoteAcmeDir} ${remoteEdgeDir}`,
])
...
run(ssh[0], [
  ...ssh.slice(1),
  host,
  `cd ${remoteEdgeDir} && docker compose config >/dev/null && docker compose run --rm --no-deps edge nginx -t`,
])
run(ssh[0], [
  ...ssh.slice(1),
  host,
...
console.log(`Deployed blog, notes, and edge routing to ${host}.`)
